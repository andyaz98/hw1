function addToFavourites(event) {
    ...
    fetch("http://localhost:8080/hw1/Script%20php/add_to_favourites.php?fav=" + fav_destination).then(onResponseFav).then(onText);
}
...
function removeFromFavourites(event){
    ...
    const fav_destination = event.currentTarget.dataset.destination;
    fetch("http://localhost:8080/hw1/Script%20php/delete_from_favourites.php?fav="+ fav_destination).then(onResponseFav).then(onText);
}
...
function onJson(json) {
    ...
    for (let favourite of json) {
        for (let destination of destinations) {
            ...
            if (favourite.favourites == destination.dataset.destination) {
...
function updateFavouritesHome() {
    fetch("http://localhost:8080/hw1/Script%20php/get_favourites.php").then(onResponseUpdate).then(onJson);
}
