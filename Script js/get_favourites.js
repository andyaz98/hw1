function deleteFavourite(event){
    ...
    fetch("http://localhost:8080/hw1/Script%20php/delete_from_favourites.php?fav="+fav_destination).then(onResponseDel).then(onText);
}
...
function onJson(json){
    ...
    for(let favourite of json){
        ...
        favourite_em.textContent=favourite.favourites;
        ...
        del_fav.dataset.destination=favourite.favourites;
...
function updateFavourites(){
    fetch("http://localhost:8080/hw1/Script%20php/get_favourites.php").then(onResponse).then(onJson);
}
